...
public class RewriteNestedLoopJoinWithConstantConditionsTest extends CrateDummyClusterServiceUnitTest {
    ...
    public void test_optimize_nestedloopjoin_to_hashjoin_when_constant_can_be_pused_down() {
        ...
        var outputs = List.of(sqlExpressions.asSymbol("doc.t1.x"), sqlExpressions.asSymbol("doc.t2.y"));
        var eval = Eval.create(nl, outputs);
        var rule = new RewriteNestedLoopJoinWithConstantConditions();
        Match<Eval> match = rule.pattern().accept(eval, Captures.empty());

        assertThat(match.isPresent(), Matchers.is(true));
        assertThat(match.value(), Matchers.is(eval));
        ...
        assertThat(result.outputs(), is(eval.outputs()));
        HashJoin hashJoin = (HashJoin) result.sources().get(0);
        assertThat(hashJoin.joinCondition(), is(nonConstantPart));
        assertThat(hashJoin.lhs(), is(c1));
        Filter filter = (Filter) hashJoin.rhs();
        assertThat(filter.source(), is(c2));
        assertThat(filter.query(), is(constantPart));
    }

    @Test
    public void test_number_of_relations() {
        Symbol constantCondition = sqlExpressions.asSymbol("true");
        assertThat(RewriteNestedLoopJoinWithConstantConditions.numberOfRelationsUsed(constantCondition), is(0));

        constantCondition = sqlExpressions.asSymbol("t1.x = 4");
        assertThat(RewriteNestedLoopJoinWithConstantConditions.numberOfRelationsUsed(constantCondition), is(1));
        ...
        assertThat(RewriteNestedLoopJoinWithConstantConditions.numberOfRelationsUsed(constantCondition), is(2));
