...
public final class MetadataTracker implements Closeable {
    ...
    private CompletableFuture<?> processSubscription(String subscriptionName) {
        // In many cases this method returns a completed future if some state is missing.
        // This is acceptable because the subscription processing is re-scheduled all the time
        //
        ...
        CompletableFuture<ClusterState> remoteClusterState = getRemoteClusterState(client);
        CompletableFuture<Boolean> thisClusterStateUpdateAcked = remoteClusterState.thenCompose(remoteState -> {
        ...
        CompletableFuture<Boolean> subscribedToNewRelations = thisClusterStateUpdateAcked.thenCompose(acked -> {
            if (!acked) {
                return CompletableFuture.completedFuture(false);
            }
        ...
        return subscribedToNewRelations.exceptionallyCompose(err -> {
